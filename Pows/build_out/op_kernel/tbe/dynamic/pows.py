...
@tbe_register.register_operator("Pows", trans_bool_to_s8=False)
...
def pows(x1_in__, x2_in__, y_out_, kernel_name="pows", impl_mode=""):
    ...
    origin_func_name = "pows"
    ascendc_src_dir_ex = "pows"
    ascendc_src_dir = "pows"
    ascendc_src_file = "pows.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    msg = "start compile Acend C Operator Pows, kernel name is " + kernel_name
    CommonUtility.print_compile_log("", msg, AscendCLogLevel.LOG_INFO)
    op_type = "Pows"
...
def op_select_format(x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_)
    result = check_op_cap("op_select_format", "Pows", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "Pows", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
